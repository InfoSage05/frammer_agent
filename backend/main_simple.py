"""
Main FastAPI server with simple architecture
"""
import sys
...
# Initialize app
app = FastAPI(title="Frammer AI Agent")
...
async def chat(request: ChatRequest):
    """Main chat endpoint - uses Master Orchestrator"""
...
async def ask_ai(request: AskAIRequest):
    """
    Ask AI endpoint - Uses RAG pipeline with Groq LLM
    Retrieves relevant context and generates intelligent answers
    """
    session_id = request.session_id or str(uuid.uuid4())
...
def generate_followup_suggestions(query: str, datasets_used: list, charts_created: list, session_memory) -> list:
