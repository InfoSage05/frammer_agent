...
export default function AnalyzeToolPage() {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent) => { e.preventDefault(); e.stopPropagation(); handleFiles(e.dataTransfer.files); },
    [handleFiles]
  );
  ...
  return (
    <div className="shell" style={{ display: "flex", minHeight: "100vh" }}>
      {/* Mini sidebar */}
      <div style={{ width: 220, minWidth: 220, background: "var(--bg2)", borderRight: "1px solid var(--line)", padding: "20px 14px", display: "flex", flexDirection: "column", gap: 12 }}>
        <a href="/" style={{ fontFamily: "var(--font-mono)", fontSize: 10, color: "var(--ink4)", textDecoration: "none", letterSpacing: "0.1em", display: "flex", alignItems: "center", gap: 6 }}>
          &larr; BACK TO DASHBOARD
        </a>
        <div style={{ fontFamily: "var(--font-serif)", fontSize: 18, color: "var(--ink)", marginTop: 8 }}>Upload &amp; Analyze</div>
        <div style={{ fontSize: 11, color: "var(--ink4)", lineHeight: 1.5 }}>Upload CSV datasets for isolated analysis. Does not affect the main dashboard.</div>

        {result && (
          <div style={{ marginTop: 16 }}>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 8, letterSpacing: "0.12em", color: "var(--ink4)", textTransform: "uppercase", marginBottom: 8 }}>ANALYSIS TABS</div>
            {SUB_TABS.map(([k, l]) => (
              <div key={k} className={`nav-item${subTab === k ? " active" : ""}`} onClick={() => setSubTab(k)} style={{ cursor: "pointer" }}>
        ...
        {result && (
          <div style={{ marginTop: "auto" }}>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 8, letterSpacing: "0.12em", color: "var(--ink4)", textTransform: "uppercase", marginBottom: 8 }}>DATASETS LOADED</div>
            {(result.files || []).map((f, i) => (
              <div key={i} style={{ fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--ink3)", padding: "3px 0", display: "flex", justifyContent: "space-between" }}>
                <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 130 }}>{f.filename}</span>
                <span style={{ fontSize: 8, color: f.status === "success" ? "var(--suc)" : "var(--red-lt)", padding: "1px 5px", background: f.status === "success" ? "rgba(48,176,96,0.1)" : "rgba(255,71,87,0.1)", borderRadius: 3 }}>
                  {f.classified_role || f.status}
                </span>
              </div>
            ))}
            <button onClick={handleReset} style={{ marginTop: 12, width: "100%", padding: "6px", fontSize: 10, fontFamily: "var(--font-mono)", background: "var(--bg3)", border: "1px solid var(--line)", color: "var(--ink3)", borderRadius: 4, cursor: "pointer", letterSpacing: "0.08em" }}>
              RESET &amp; NEW UPLOAD
            </button>
          </div>
        )}
        ...
        {!result && (
          <div style={{ maxWidth: 700, margin: "60px auto" }}>
            <div style={{ textAlign: "center", marginBottom: 32 }}>
              <div style={{ fontFamily: "var(--font-serif)", fontSize: 28, color: "var(--ink)", marginBottom: 8 }}>Upload Datasets</div>
              <div style={{ fontSize: 13, color: "var(--ink4)" }}>Drop your CSV files below to run isolated analytics</div>
            </div>
            <div ref={dropRef} onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()}
              style={{ border: "2px dashed var(--line)", borderRadius: 12, padding: "48px 24px", textAlign: "center", cursor: "pointer", background: "var(--bg3)", transition: "border-color 0.2s" }}
              onMouseEnter={(e) => (e.currentTarget.style.borderColor = "var(--pri)")} onMouseLeave={(e) => (e.currentTarget.style.borderColor = "var(--line)")}>
              <div style={{ fontSize: 36, marginBottom: 12, opacity: 0.4 }}>+</div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 12, color: "var(--ink3)", letterSpacing: "0.08em" }}>DRAG &amp; DROP CSV FILES HERE</div>
              <div style={{ fontSize: 11, color: "var(--ink4)", marginTop: 6 }}>or click to browse</div>
            ...
            {files.length > 0 && (
              <div style={{ marginTop: 20 }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.1em", color: "var(--ink4)", textTransform: "uppercase", marginBottom: 8 }}>SELECTED FILES ({files.length})</div>
                {files.map((f, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: "var(--bg3)", borderRadius: 6, marginBottom: 4, border: "1px solid var(--line-lt)" }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--ink2)" }}>{f.name}</span>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontFamily: "var(--font-mono)", fontSize: 9, color: "var(--ink4)" }}>{(f.size / 1024).toFixed(0)} KB</span>
                      <button onClick={(e) => { e.stopPropagation(); removeFile(i); }} style={{ background: "none", border: "none", color: "var(--red-lt)", cursor: "pointer", fontSize: 14, lineHeight: 1, padding: 0 }}>&times;</button>
                    </div>
                  </div>
                ))}
                <button onClick={handleAnalyze} disabled={analyzing} style={{ marginTop: 16, width: "100%", padding: "12px", fontSize: 13, fontFamily: "var(--font-mono)", fontWeight: 600, background: analyzing ? "var(--bg3)" : "var(--pri)", color: analyzing ? "var(--ink3)" : "#fff", border: "none", borderRadius: 8, cursor: analyzing ? "wait" : "pointer", letterSpacing: "0.1em", transition: "all 0.2s" }}>
                  {analyzing ? "ANALYZING..." : "ANALYZE DATASETS"}
                </button>
              </div>
            )}
            {error && (
              <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(255,71,87,0.08)", border: "1px solid rgba(255,71,87,0.2)", borderRadius: 8, color: "var(--red-lt)", fontFamily: "var(--font-mono)", fontSize: 11 }}>{error}</div>
            )}
          </div>
        )}
...
function BreakdownTab({ exec, explorer, result }: any) {
  ...
  const catColors = {
    volume: "var(--ink)", conversion: "var(--pri)", efficiency: "var(--amber)",
    duration: "var(--ink3)", growth: "var(--suc)", content_mix: "var(--pri)",
    platform: "var(--amber)", data_quality: "var(--red-lt)",
  };
        ...
        <div className="card" style={{ padding: "20px 24px" }}>
          ...
          {Object.entries(grouped).map(([cat, items]) => (
            <div key={cat} style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10, paddingBottom: 6, borderBottom: `2px solid color-mix(in srgb, ${catColors[cat] || "var(--ink3)"} 35%, transparent)` }}>
